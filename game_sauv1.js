...
static combatTable = [
    ...
    static playerGenerator(name)
    {
        ...
        this.weaponChoice(player);
        this.disciplineChoice(player);
    ...
    static loadPlayer(saveName = "autosave") {
        ...
        return {
            ...
            tabDiscipline: data.disciplines,
            nbrDiscipline: data.disciplines.filter(Boolean).length,
            tabWeapon: data.weapons,
            nbrWeapon: data.weapons.filter(Boolean).length,
            weaponskillWeapon: data.weaponskillWeapon,
            bag: data.bag,
            combat: false
        };
    }

    static weaponChoice(player)
    {
        const weaponNames = [
        ...
        // player.nbrWeapon < Game.WEAPON_MAX
        while (player.nbrWeapon < (Game.WEAPON_MAX-1)) {
            console.log("------------------- PLAYER --------------------");
            console.log("Choisissez 2 armes parmi les suivantes :\n");
            weaponNames.forEach((name, index) => {
                console.log(`${index + 1}- ${name}`);
            });
            console.log(`\nNombre d'armes possédées : ${player.nbrWeapon}`);
            console.log("----------------------------------------------");
            
            const choice = parseInt(prompt("--> ")) - 1;
            
            if (isNaN(choice) == false) {
                console.clear();
                if (choice >= 0 && choice < 10 && player.tabWeapon[choice] == false) {
                    player.tabWeapon[choice] = true;
                    player.nbrWeapon++;
                    // break;
                } else {
                    console.log("------------------------------------");
                    console.log("| Choix invalide ou déjà possédé ! |");
                    console.log("------------------------------------\n");
                }
            }
        }
    ...
    static disciplineChoice(player)
    {
        ...
        while (player.nbrDiscipline < 6) {
            console.log("------------------- PLAYER --------------------");
            console.log("Choisissez 6 disciplines parmi les suivantes :\n");
            disciplineNames.forEach((name, index) => {
                console.log(`${index + 1}- ${name}`);
            });
            console.log(`\nNombre de disciplines possédées : ${player.nbrDiscipline}`);
            console.log("----------------------------------------------");
            ...
            if (!isNaN(choice)) {
                console.clear();
                if (choice >= 0 && choice < 10 && !player.tabDiscipline[choice]) {
                    player.tabDiscipline[choice] = true;
                    player.nbrDiscipline++;
                    
                    if (choice === Game.Disciplines.WEAPONSKILL) {
                        player.weaponskillWeapon = Game.generateRnt();
                        console.log(`[Le Bonus s'appliquera à l'arme : ${weaponNames[player.weaponskillWeapon]}]\n`);
                        
                        if (player.tabWeapon[player.weaponskillWeapon]) {
                            console.log("[Arme Possédée, +2 en habileté en combat si équipée]\n");
                        }
                    }
                } else {
                    console.log("------------------------------------");
...
const player = Game.playerGenerator("Aragorn");
const enemy = Game.playerGenerator("Orc");
...
function renderDisciplineChoices() {
    ...
    disciplines.forEach((discipline, i) => {
        ...
        // Désactiver si déjà choisi ou limite atteinte
        if (player.tabDiscipline[i] || disciplinesChosen >= MAX_DISCIPLINES) {
